...
namespace uDicom.WorkQueue.Service.ShredHost
{
	...
	public abstract class QueueProcessorShred : Shred
    {
        ...
        private void StartUp()
        {
            ...
            Platform.Log(LogLevel.Info, string.Format("Starting {0} Shred", this.GetDisplayName()));
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                if (!StopRequested && processors != null)
                {
                    try
                    {
                        ...
                        Platform.Log(LogLevel.Info, string.Format("The {0} shred has started successfully.",
                            this.GetDisplayName()));
                    }
                    catch (Exception e)
                    {
                        Platform.Log(LogLevel.Error, e, string.Format("The {0} shred has failed to start.  Please check the log for more details.", 
                            this.GetDisplayName()));
        ...
        private void ShutDown()
        {
            ...
            Platform.Log(LogLevel.Info, string.Format("Stopping {0} Shred", this.GetDisplayName()));
            ...
            Platform.Log(LogLevel.Info, string.Format("The {0} shred has stopped successfully.", 
                this.GetDisplayName()));
