...
namespace uDicom.WorkQueue.Service.ShredHost
{
    public abstract class WcfShred : Shred, IWcfShred
    {
        ...
        public ServiceEndpointDescription StartHttpHost<TServiceType, TServiceInterfaceType>
            (string name, string description)
        {
            ...
            Platform.Log(LogLevel.Info, "WCF Shred {0} is listening at {1}.", 
                name, sed.ServiceHost.Description.Endpoints[0].Address);

            return sed;
        }

        public ServiceEndpointDescription StartBasicHttpHost<TServiceType, TServiceInterfaceType>
            (string name, string description)
        {
            Platform.Log(LogLevel.Info, "Starting WCF Shred {0}...", name);
            ...
            ServiceEndpointDescription sed =
                WcfHelper.StartBasicHttpHost<TServiceType, TServiceInterfaceType>(
                    name, description, SharedHttpPort, ServiceAddressBase);
            _serviceEndpointDescriptions[name] = sed;
        ...
        public ServiceEndpointDescription StartHttpDualHost<TServiceType, TServiceInterfaceType>
            (string name,string description)
        {
            Platform.Log(LogLevel.Info, "Starting WCF Shred {0}...", name);
            ...
            ServiceEndpointDescription sed =
                WcfHelper.StartHttpDualHost<TServiceType, TServiceInterfaceType>(
                    name, description, SharedHttpPort, ServiceAddressBase);
            _serviceEndpointDescriptions[name] = sed;
                                                                                               ...
                                                                                               string description)
        {
            ...
            ServiceEndpointDescription sed =
                WcfHelper.StartNetTcpHost<TServiceType, TServiceInterfaceType>(
                    name, description, SharedTcpPort, SharedHttpPort, ServiceAddressBase);
            _serviceEndpointDescriptions[name] = sed;
                                                                                                ...
                                                                                                string description)
        {
            ...
            ServiceEndpointDescription sed =
                WcfHelper.StartNetPipeHost<TServiceType, TServiceInterfaceType>(
                    name, description, SharedHttpPort, ServiceAddressBase);
            _serviceEndpointDescriptions[name] = sed;
        ...
        protected void StopHost(string name)
        {
            ...
            else
            {
                ...
                Platform.Log(LogLevel.Debug, "Attempt to stop WCF Shred {0} failed: shred doesn't exist.", 
                    name);
            }
        }
